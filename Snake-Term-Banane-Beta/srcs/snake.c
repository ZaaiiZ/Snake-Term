...
{
	do {
		banana->x = 9 + rand() % (TX - 11);
		banana->y = 9 + rand() % (TY - 11);
	}	while (isOnSnake(snake, banana->x, banana->y) == 1);
	return ;
}
...
void	mooveSnake(t_snake **snake, t_snake **head)
{
	...
	*head = newHead;
	...
	drawCellChar(newHead->x, newHead->y, newHead->color, "  ");
...
{
	if (banana.x == snake->x && banana.y == snake->y)
		return (1);
	return (0);
}
